    ...
    return (
        <div className='theme'>
            ...
            <div className='theme__text text'>
                ...
                <p className='text__item text__item_padding'>Виртуальный DOM (VDOM) — это концепция программирования, в которой «виртуальное» представление пользовательского интерфейса хранится в памяти в виде легковестной копии DOM и синхронизируется с «настоящим» DOM при помощи библиотеки, такой как ReactDOM.Этот процесс называется согласованием (Reconciliation).
                </p>
                <p className='text__item text__item_padding'>Процесс согласования(Reconciliation) другими словами - это процесс отслеживания изменений в состоянии компоненов и оторажения обновленного состояния на экране с минимальными трудозатратами.
                </p>
                <p className='text__item text__item_padding'>При изменении состояния любого элемента, создается новое VDOM дерево.После этого вычисляется разница между предыдущей и текущей версиями VDOM деревьев. Этот процесс называется «поиском различий» (diffing). Цель данных вычислений состоит в минимизации количества операций, совершаемых с DOM. Затем все родительское поддерево VDOM подвергается повторному рендерингу. Наконец, это обновленное дерево используется для обновления DOM.</p>
                ...
                <p className='text__item text__item_padding'>React реализует эвристический алгоритм для вычислений (Эвристический алгоритм — это алгоритм решения задачи, правильность которого для всех возможных случаев не доказана, но про который известно, что он даёт достаточно хорошее решение в большинстве случаев.), который основывается на двух утверждениях:
                    <ol>
                        <li>1. Два элемента с разными типами произведут разные деревья.</li>
                        <li>2. Разработчик может указать какие дочерние элементы останутся стабильными между рендерами с помощью пропа key.</li>
                    </ol></p>
